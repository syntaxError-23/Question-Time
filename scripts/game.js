...
// -------------------- DOM VARIABLES -------------------- 
const gridItems = document.querySelectorAll('.grid-item'); //Individual Grid items (as nodelist)
const confirmChoiceBtn = document.getElementById('confirmChoicebtn'); //confirm choice (grid)
const confirmAnsBtn = document.getElementById('confirmAnsBtn'); //Confirm answer button
const choicesArea = document.getElementById('choices'); //choices (grid) section
...
// --------------- QUICK ROUND FUNCTIONS ---------------

const selectGridItem = (item => {
...
if(gameMode === 'quickround'){
    ...
    gridItems.forEach(selectGridItem);
    answerOptions.forEach(selectAnswer);
    ...
    let randomIndex = Math.floor(math.random * infModeQuestions)
    let currentQ = infModeQuestions[randomIndex]
    
    const infModeQuestions = [...questionsObjArr]
    ...
    setTimeout(() => {
        questionsArea.classList.toggle('transparent');
    }, 200)
    }
